---
title: "はじめに ── 序説という位置づけについて"
free: true
---

## なぜ「序説」か

本書を「全説」ではなく「序説」と名付けたのは、これが出発点だからだ。

thought-analyzerはデータの蓄積とともに進化する。現時点では9軸・6軸の分析基盤を提供しているが、データが集積することで比較・独自性評価・応用分析が可能になる。それらは別の一冊に委ねる。本書は、その起点となる設計・仕様・基礎的な活用法を記述した序章だ。

「序説」という言葉は「不完全」を意味しない。基礎を正確に記述することが、応用への確かな足場になる。
